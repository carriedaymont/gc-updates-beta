...
# ---------------------------------------------------------------------------
# Test 4 removed (2026-06-24): it asserted exclude == "Exclude-C-PIV", but the
# default `exclude` column emits the Summary code "Exclude-Hard-Limit" (the internal
# "Exclude-C-PIV" is mapped on output), so both counts were 0 and the test
# passed vacuously. Constructed-subject coverage of the PIV cutoffs + 1-year
# split is being added under the Tier 1 testing-gaps work (T18/T19).
# ---------------------------------------------------------------------------
...
test_that("error load parameters affect error-load exclusions", {
  ...
  n_errload_agg <- sum(out_aggressive$result$exclude == "Exclude-C-Unevaluable-Trajectory")
  n_errload_len <- sum(out_lenient$result$exclude == "Exclude-C-Unevaluable-Trajectory")

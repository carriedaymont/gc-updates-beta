...
# cleanadult() is internal (not exported); reference it via the package
# namespace. This harness runs against the INSTALLED package (like the testthat
# suite), so reinstall after editing adult code before running it.
cleanadult <- growthcleanr:::cleanadult
...
# Load data
input_file <- system.file("testdata", "adult-gc-test-ALL-PHASES.csv", package = "growthcleanr")
df <- fread(input_file)

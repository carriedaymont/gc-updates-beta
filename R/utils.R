...
#'
#' @param long_df A data frame (or data.table) to be transformed. Expects
#' columns: id, subjid, sex, agedays, param, measurement, and an exclusion
#' column from \code{cleangrowth()} (named \code{"exclude"} by default; the
#' \code{cleangrowth()} output \code{data.table} can be passed directly).
...
#' Defaults to \code{"exclude"} (the column name in v3.0.0+ \code{cleangrowth()}
#' output). For data prepared with a differently-named exclusion column (e.g.,
#' from older code paths), pass the column name explicitly.
...
#' df <- syngrowth[syngrowth$subjid %in% unique(syngrowth$subjid)[1:2], ]
#' gc_out <- cleangrowth(df)
#' df$exclude <- gc_out$exclude[match(df$id, gc_out$id)]
#'
#' # Convert to wide format (default gcr_result = "exclude" matches v3.0.0+)
#' wide_df <- longwide(df)
#'
#' # Include all inclusion types
#' wide_df <- longwide(df, include_all = TRUE)
#'
#' # Specify which exclusion codes to retain (default `exclude` column uses
#' # the Summary codes; pass `exclude_detail = TRUE` to cleangrowth() and use
#' # `gcr_result = "exclude_detailed"` for the step-level Detailed codes)
#' wide_df <- longwide(df, inclusion_types = c("Include", "Exclude-CF"))
#' }
longwide <-
  function(long_df,
           id = "id",
    ...
    if (!requireNamespace("dplyr", quietly = TRUE) ||
      ...
      stop("longwide() requires the 'dplyr' and 'tidyr' packages. Install them with install.packages(c(\"dplyr\", \"tidyr\")).")
    }

    # Accept data.table input (e.g. cleangrowth() output, which is a data.table):
    # this function uses data.frame-style `[, col]` indexing throughout, where a
    # data.table would instead read `col` as a literal column name. Coerce up front.
    ...
    if(all(c(id, subjid, sex, agedays, param, measurement, gcr_result,
      ...
      obs_df <- dplyr::select(long_df, id, subjid, sex, agedays, param, measurement,
                              dplyr::all_of(gcr_result), dplyr::all_of(extra_cols))
    } else {
    ...
    obs_df <- dplyr::mutate(obs_df, sex = sex_recoded)
    obs_df <- dplyr::mutate(obs_df, param = as.character(param))
    clean_df <- dplyr::select(obs_df, subjid, id, agey, agem, agedays, sex, param,
                              measurement, dplyr::all_of(extra_cols))
    ...
    param_separated <- tidyr::pivot_wider(clean_df, names_from = param,
                                          values_from = measurement)

    # extract heights and weights attached to ids
    height <- dplyr::filter(param_separated, !is.na(HEIGHTCM))
    height <- dplyr::filter(height, is.na(WEIGHTKG)) # why this, shouldn't the !is.na height take care of it
    height <- dplyr::mutate(height, ht_id = id)
    height <- dplyr::select(height, -id)
    height <- dplyr::select(height, -WEIGHTKG)

    weight <- dplyr::filter(param_separated, is.na(HEIGHTCM))
    weight <- dplyr::filter(weight, !is.na(WEIGHTKG))
    weight <- dplyr::mutate(weight, wt_id = id)
    weight <- dplyr::select(weight, -id)
    weight <- dplyr::select(weight, -HEIGHTCM)
    ...
    wide_df <- dplyr::mutate(wide_df, wt = WEIGHTKG, ht = HEIGHTCM)
    ...
    wide_df <- dplyr::select(wide_df, -c(HEIGHTCM, WEIGHTKG))
...
#' # cleangrowth() returns the data with an 'exclude' column (the v3.0.0+ default)
#' clean_df <- cleangrowth(df)
#' # Convert to wide format
#' wide_df <- longwide(clean_df)
